...
3. Source Transformer: add a JavaScript step and paste the **PD_Request_In → Source Transformer** code. The transformer sets `responseStatusCode`/`responseHeaders` and the return value serves as the HTTP response body (empty string yields an empty response body).
...
4. Deploy. The transformer writes HL7 v3 XML to `responseMessage` and sets `Content-Type` to `application/xml` with HTTP 202; the source transformer return value is the HTTP body.
